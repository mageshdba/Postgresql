...
//FAILOVER
========================================
Source	    : 172.184.129.58
Destination : 173.11.85.62
-------------------
//PROMOTE DR
-------------------
...
mv /dbdata/pgsql/ /dbdata/pgsql_old/
mkdir /dbdata/pgsql
chown -R postgres:postgres /dbdata/pgsql
-------------------
//Enable Replication (pg_basebackup) 
-------------------


nohup sudo -u postgres PGPASSWORD=Repl@123 pg_basebackup --pgdata /dbdata/pgsql/ --format=p --write-recovery-conf --checkpoint=fast --label=mffb --progress --host=172.184.129.58 --port=5665 --username=replusr &  
...
--On PR
select * from pg_create_physical_replication_slot('repl_slot_2');
...
//On Standby
select sender_host,status,last_msg_send_time from pg_stat_wal_receiver;
