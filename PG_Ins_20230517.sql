...
mkdir -p /dblog/pgsql
mkdir -p /dberrorlog/pgsql/
mkdir -p /dberrorlog/pgsql/audit/old_logs_audit
mkdir -p /dbdata01/tmp/
...
chown -R postgres:postgres /dbdata01/tmp/
chown -R postgres:postgres /dblog/pgsql/archive


#--INIT DB

sudo /usr/pgsql-14/bin/pgsqlql-14-setup initdb 
...
CREATE USER m2pappusr WITH createdb PASSWORD 'Mshbgv23t24@332';
...
sudo rsync -avhz /var/lib/pgsql/14/data/ /dbdata01/pgsql/


cp -p /var/lib/pgsql/14/data/postgresql.conf	 /var/lib/pgsql/14/data/postgresql.conf_`date +%Y_%m_%d.%H.%M.%S`
rm  /var/lib/pgsql/14/data/postgresql.conf	


vi  /var/lib/pgsql/14/data/postgresql.conf	

data_directory = '/dbdata01/pgsql' # use data in another directory
...
4GB RAM / 4 CORE CPU
data_directory = '/dbdata01/pgsql/data' # use data in another directory
listen_addresses = '*'              # what IP address(es) to listen on;
port = 4554
...
cat /var/lib/pgsql/14/data/postgresql.conf_bkp |grep '^[[:blank:]]*[^[:blank:]#;]' > /var/lib/pgsql/14/data/postgresql.conf

chown postgres:postgres /var/lib/pgsql/14/data/postgresql.conf


/var/lib/pgsql/14/data/postgresql.conf_`date +%Y_%m_%d.%H.%M.%S`
...
log_directory = '/dberrorlog/pgsql/'                 # directory where log files are written,
...
primary_conninfo = 'user=replusr port=5665 host=172.184.45.36 application_name=172.184.45.36.replusr'                  
primary_slot_name = 'standby_repl_slot'                   

pgaudit.log_directory='/dberrorlog/pgsql/audit'
...
echo "host    replication     replusr             173.11.37.82/32            scram-sha-256"   >> /var/lib/pgsql/14/data/pg_hba.conf
...
echo "local   all             all                                     peer"                   >> /var/lib/pgsql/14/data/pg_hba.conf
echo "host    all             all             0.0.0.0/0            scram-sha-256"             >> /var/lib/pgsql/14/data/pg_hba.conf
echo "host    replication     replusr             172.184.45.36/32            scram-sha-256"  >> /var/lib/pgsql/14/data/pg_hba.conf
...
host    all             all             0.0.0.0/0            scram-sha-256
host    replication     replusr             172.184.45.36/32            scram-sha-256
...
select slot_name, slot_type, active, wal_status from pg_replication_slots;
select * from pg_create_physical_replication_slot('standby_repl_slot');

--to drop
select pg_drop_replication_slot('pg01_repl_slot');
...
-----------------------------
VERSION 14
sudo -u postgres pg_basebackup --pgdata /dbdata01/pgsql/ --format=p --write-recovery-conf --checkpoint=fast --label=mffb --progress --host=172.184.45.36 --port=5665 --username=replusr

OR

sudo -u postgres pg_basebackup -h 174.1.0.6 -D /dbdata01/pgsql/ -U replication -v -P --xlog-method=stream 
pg_basebackup -h 174.1.0.6 -D /dbdata01/pgsql/ -U replication -v -P --xlog-method=stream 
pg_basebackup -h 174.1.0.6 -U replication -D /dbdata01/pgsql/
pg_basebackup -h 174.1.0.6 -U replication --checkpoint=fast \ -D /dbdata01/pgsql/ -R --slot=some_name -C
pg_basebackup -h 174.1.0.6 -U replication -D /dbdata01/pgsql/ --checkpoint=fast -R --slot=some_name -C
...
Node 3
pg_basebackup -h 174.1.0.6 -U replication -D /dbdata01/pgsql/ --checkpoint=fast -R --slot=some_name2 -C
